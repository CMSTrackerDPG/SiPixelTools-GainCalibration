...
luminosity in between the `2025_v1` [elog](https://cmsonline.cern.ch/cms-elog/1269682) and `2026_v1` [elog](https://cmsonline.cern.ch/cms-elog/1324134) gain calibration
tags. This is best done in a separate terminal connected to lxplus:
```
source /cvmfs/cms-bril.cern.ch/cms-lumi-pog/brilws-docker/brilws-env

brilcalc lumi -u /fb --normtag /cvmfs/cms-bril.cern.ch/cms-lumi-pog/Normtags/normtag_BRIL.json --begin "06/27/25 00:00:00" --end "02/23/26 12:00:00"
```
...
The obtained amount of delivered integrated luminosity in this particular case is approximately 99.79 /fb. This amount has been added to
[`test/calc_corrections.py`](test/calc_corrections.py) with the following line
```
lumi["2026_v1"] = lumi["2025_v1"] + 99.793576820  # in fb^{-1}
```
...
```
values used for 2026_v1 new model:
corrs_bpix = {1: 1.101, 2: 1.065, 3: 1.047, 4: 1.029}
corrs_fpix = {1: 1.1449, 2: 1.1004}
```
and also produces an output file named `calc_corrections_2026_v1.png` that contain a graph of received doses (in Mrad) for different parts of the pixel detector.

To get the updated database object, run
```
cmsRun SiPixelVCalDB_cfg.py tagName=SiPixelVCal_phase1_2026_v1
```
This produces the output file named `SiPixelVCal_phase1_2026_v1.db`. After producing the file, please update the corresponding [TWiki](https://twiki.cern.ch/twiki/bin/viewauth/CMS/SiPixelVCalHistory).
...
```
cp -v /eos/cms/store/group/dpg_tracker_pixel/comm_pixel/GainCalibrations/Phase1/Run_11173/GainCalibration_1205_11173.dmp ./
cmsRun gain_calib_cfg.py vcalTag=SiPixelVCal_phase1_2026_v1 run=11173 fed=1205
```
...
For example,
```
./run.py create 11173 -t SiPixelVCal_phase1_2026_v1 -i /eos/cms/store/group/dpg_tracker_pixel/comm_pixel/GainCalibrations/Phase1/Run_11173 -o /eos/cms/store/group/dpg_tracker_pixel/comm_pixel/$USER/
```
To run from the default `Phase1/Run_*` directory, which is what you will do for any "real" gain calibration, simply do
```
./run.py create 11173 -t SiPixelVCal_phase1_2026_v1
```
...
```
./run.py create 11173 -t SiPixelVCal_phase1_2026_v1 --BPix-only
```
...
```
getPayloadData.py --plugin pluginSiPixelGainCalibrationOffline_PayloadInspector --plot plot_SiPixelGainCalibOfflineGainComparisonBarrelTwoTags --tag SiPixelGainCalibration_2025_v1 --tagtwo SiPixelGainCalibration_2026_v1 --time_type Run --iovs '{"start_iov": "1", "end_iov": "1"}' --iovstwo '{"start_iov": "1", "end_iov": "1"}' --db Prod --test

getPayloadData.py --plugin pluginSiPixelGainCalibrationOffline_PayloadInspector --plot plot_SiPixelGainCalibOfflinePedestalComparisonBarrelTwoTags --tag SiPixelGainCalibration_2025_v1 --tagtwo SiPixelGainCalibration_2026_v1 --time_type Run --iovs '{"start_iov": "1", "end_iov": "1"}' --iovstwo '{"start_iov": "1", "end_iov": "1"}' --db Prod --test

getPayloadData.py --plugin pluginSiPixelGainCalibrationOffline_PayloadInspector --plot plot_SiPixelGainCalibOfflineGainComparisonEndcapTwoTags --tag SiPixelGainCalibration_2025_v1 --tagtwo SiPixelGainCalibration_2026_v1 --time_type Run --iovs '{"start_iov": "1", "end_iov": "1"}' --iovstwo '{"start_iov": "1", "end_iov": "1"}' --db Prod --test

getPayloadData.py --plugin pluginSiPixelGainCalibrationOffline_PayloadInspector --plot plot_SiPixelGainCalibOfflinePedestalComparisonEndcapTwoTags --tag SiPixelGainCalibration_2025_v1 --tagtwo SiPixelGainCalibration_2026_v1 --time_type Run --iovs '{"start_iov": "1", "end_iov": "1"}' --iovstwo '{"start_iov": "1", "end_iov": "1"}' --db Prod --test
```
The above plots compare gains and pedestals in BPix and FPix between the `2025_v1` and `2026_v1` gain calibrations.
