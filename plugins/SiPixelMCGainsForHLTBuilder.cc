#include <memory>
#include <iostream>
#include "SiPixelMCGainsForHLTBuilder.h"
...
#include "CLHEP/Random/RandFlat.h"
  ...
  SiPixelMCGainsForHLTBuilder::SiPixelMCGainsForHLTBuilder(const edm::ParameterSet& iConfig)
      : conf_(iConfig),
  ...
  void SiPixelMCGainsForHLTBuilder::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    using namespace edm;
    ...
    edm::LogInfo("SiPixelMCGainsForHLTBuilder")
    ...
    edm::LogInfo("SiPixelMCGainsForHLTBuilder") << " There are " << pDD->dets().size() << " detectors" << std::endl;
   //Retrieve tracker topology from geometry
    ...
    for (TrackerGeometry::DetContainer::const_iterator it = pDD->dets().begin(); it != pDD->dets().end(); it++) {
      ...
      // Find out if it is layer 1
        ...
        if (!SiPixelGainCalibration_->put(detid, range, ncols))
          edm::LogError("SiPixelMCGainsForHLTBuilder")
              << "[SiPixelMCGainsForHLTBuilder::analyze] detid already exists" << std::endl;
      }
    }
    ...
    try {
      ...
      //     edm::LogInfo("SiPixelMCGainsForHLTBuilder")<<"CallbackToken SiPixelGainCalibration "
      ...
      //     edm::LogInfo("SiPixelMCGainsForHLTBuilder")<<"[SiPixelMCGainsForHLTBuilder::analyze] tillTime = "
      //         <<tillTime<<std::endl;
      ...
      edm::LogError("SiPixelMCGainsForHLTBuilder") << er.what() << std::endl;
    } catch (const std::exception& er) {
      edm::LogError("SiPixelMCGainsForHLTBuilder") << "caught std::exception " << er.what() << std::endl;
    } catch (...) {
      edm::LogError("SiPixelMCGainsForHLTBuilder") << "Funny error" << std::endl;
    }
  }

  // ------------ method called once each job just before starting event loop  ------------
  void SiPixelMCGainsForHLTBuilder::beginJob() {
    if (fromFile_) {
      if (loadFromFile()) {
        edm::LogInfo("SiPixelMCGainsForHLTBuilder") << " Calibration loaded: Map size " << calmap_.size() << " max "
  ...
  void SiPixelMCGainsForHLTBuilder::endJob() {}

  bool SiPixelMCGainsForHLTBuilder::loadFromFile() {
    ...
    if (in_file.bad()) {
      edm::LogError("SiPixelMCGainsForHLTBuilder") << "Input file not found" << std::endl;
    }
    if (in_file.eof() != 0) {
      edm::LogError("SiPixelMCGainsForHLTBuilder") << in_file.eof() << " " << in_file.gcount() << " " << in_file.fail()
    ...
    for (int i = 0; i < 3; i++) {
      ...
      edm::LogInfo("SiPixelMCGainsForHLTBuilder") << line << std::endl;
    }
  ...
  DEFINE_FWK_MODULE(SiPixelMCGainsForHLTBuilder);
